	      ...
	      <ul class="nav navbar-nav navbar-right">
    		...
    		<c:if test="${!empty mvo}">
				<c:if test="${empty mvo.memProfile}">
		  			<li><img class="img-circle" src="${contextPath}/resources/images/person.png" style="width: 50px; height: 50px;" /> ${mvo.memName}
		  			(<c:forEach var="authVO" items="${mvo.authList}">
		  					<c:if test="${authVO.auth eq 'ROLE_USER'}">유저</c:if> 
